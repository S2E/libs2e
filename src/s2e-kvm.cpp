...
#ifdef CONFIG_SYMBEX
const char *g_s2e_shared_dir = nullptr;
#endif
...
void S2EKVM::Init(void) {
    x86_cpudef_setup();
    if (cpu_x86_register(&m_cpuid, s_cpuModel) < 0) {
    ...
    s2e_initialize(argc, argv, tcg_llvm_ctx, config_file, output_dir, unbuffered_stream, 0, max_processes);
