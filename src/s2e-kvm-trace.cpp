...
void *KVMTraceVCPU::sc_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset) {
    return g_original_mmap64(addr, len, prot, flags, m_kvm_vcpu_fd, offset);
}

int KVMTraceVCPU::ioctl(int fd, int request, uint64_t arg1) {
    int ret = -1;
    switch ((uint32_t) request) {
        case KVM_GET_CLOCK: {
