...
VCPU::VCPU(std::shared_ptr<S2EKVM> &kvm, std::shared_ptr<VM> &vm, kvm_run *buffer) {
    ...
    g_syscalls.onExit.connect(sigc::mem_fun(*this, &VCPU::RequestProcessExit));
    g_syscalls.onSelect.connect(sigc::mem_fun(*this, &VCPU::RequestExit));
...
void *VCPU::sc_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset) {
